...
def _build_handler(method: str, path: str, body: bytes = b"", headers: Optional[Dict[str, str]] = None) -> DeepPlanHandler:
...
def parse_http_base_url(base_url: str) -> Tuple[str, int]:
    ...
    if parsed.scheme == "https":
        raise ValueError("DeepPlanClient currently supports plain HTTP transport")
    if not parsed.hostname:
        raise ValueError("base URL host is required")
    return parsed.hostname, int(parsed.port or 80)


def build_client(config: ClientConfig) -> DeepPlanClient:
    if config.mode == "http":
        host, port = parse_http_base_url(config.base_url)
        return DeepPlanClient.from_http(host=host, port=port)
    if config.mode == "in-process":
        import deepplan

        deepplan.ensure_state()
        return DeepPlanClient(transport=in_process_transport)
