# DeepPlan Agents Skill Model

This document proposes a skill model for the separate `deepplan-agents` repo.

It does not change the DeepPlan kernel boundary.
DeepPlan remains `plan-only`.
Skills belong to the orchestration/runtime layer around DeepPlan, not to the kernel itself.

## Thesis

`deepplan-agents` should be:
...
- DeepPlan owns plan state and mutation contracts
...
## Why Not Put Skills In DeepPlan
...
They belong in `deepplan-agents`.

DeepPlan should only expose the stable planning surfaces that skills consume:
...
Capabilities should stay authoritative in DeepPlan-facing contracts.
...
`deepplan-agents` should explicitly track both:
...
```json
{
  ...
  "version": "v1",
  "role": "reviewer",
  "summary": "Prioritize and resolve open review work without bypassing DeepPlan review semantics.",
...
The `deepplan-agents` repo should enforce these rules:

- skills cannot grant capabilities
- skills cannot directly redefine DeepPlan contracts
...
The first usable milestone for `deepplan-agents` should be:
...
5. keep all DeepPlan writes routed through one adapter
...
`deepplan` is not skill-based today, and that is correct.

The right move is:

- keep DeepPlan contract-first
- make `deepplan-agents` skill-aware
